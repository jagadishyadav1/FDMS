...
<?php
  ...
  $sql="select * from user_request_details where User_request_id  ='$id'";
...
<form name="form1" id="formID" method="post" action="Vendor_Request_update.php">
  ...
  {
  ?>
      <option value="<?php echo $row2['User_id'];?>"<?php if($row2['User_id']==$row['User_id']) {?>selected <?php } ?>><?php echo $row2['User_name'];?></option>
      <?php 
  }
  ?>
    </select>
  ...
  $sql4="select * from vehicle_details";
  $res4=mysqli_query($conn,$sql4);
  while($row4=mysqli_fetch_array($res4))
  {
  ?>
      <option value="<?php echo $row4['Vehicle_id'];?>"<?php if($row4['Vehicle_id']==$row['Vehicle_id']) {?>selected <?php } ?> ><?php echo $row4['Vehicle_no'];?></option>
      <?php 
  }
  ?>
    </select>
